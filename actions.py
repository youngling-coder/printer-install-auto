...
def install_printer(printer, port_name=None):
    ...
    if proceed_installation:
        ...
        port_command = (
            ...
            f"-a -r {port_name} -h {printer.ip} -o raw -n 9100"
        )
        run_command(port_command)

        # Step 2: Add driver using INF
        driver_command = f'pnputil /add-driver "{printer.driver_inf_path}" /install'
        run_command(driver_command)

        # Step 3: Install printer
        install_command = (
            f'rundll32 printui.dll,PrintUIEntry /if /b "{printer.name}" /r "{port_name}" '
            f'/f "{printer.driver_inf_path}" /m "{printer.driver_name}" /z'
        )
        run_command(install_command)

        print(f"✅ Printer '{printer.name}' installed successfully!")
